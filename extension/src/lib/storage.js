...
export async function markUrlsSeen(urls) {
  ...
  for (const url of urls) seen[url] = Date.now();
  await setLocal({ [KEYS.SEEN_URLS]: seen });
}

export async function getScanState() {
  const data = await getLocal([KEYS.SCAN_STATE]);
  return data[KEYS.SCAN_STATE] || {
    isRunning: false, lastScanAt: null, lastError: null,
    lastAddedCount: 0, totalScans: 0
  };
}

export async function setScanState(statePatch) {
  const current = await getScanState();
  await setLocal({ [KEYS.SCAN_STATE]: { ...current, ...statePatch } });
}
...
export async function clearAllData() {
  await chrome.storage.local.remove([KEYS.OPPORTUNITIES, KEYS.SEEN_URLS, KEYS.SCAN_STATE]);
}
