...
function countMatches(text, keywords) {
  ...
  for (const term of keywords) {
    if (text.includes(term)) { count++; matched.push(term); }
  }
...
export function scoreOpportunity(rawItem, settings) {
  ...
  if (high.count > 0 && countMatches(title, HIGH_WEIGHT_TERMS).count > 0) score += 15;
  ...
  if (type === OPPORTUNITY_TYPES.TENDER || type === OPPORTUNITY_TYPES.CONSULTANCY || type === OPPORTUNITY_TYPES.TRAINING) score += 10;

  const postedDate = rawItem.postedDate ? Date.parse(rawItem.postedDate) : null;
  if (postedDate && !Number.isNaN(postedDate)) {
    const ageDays = (Date.now() - postedDate) / 86400000;
